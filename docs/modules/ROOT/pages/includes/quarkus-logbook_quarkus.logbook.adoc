...
[.description]
--
The strategy to use for logging requests and responses. Valid values are:

 - `default`: Log everything.
 - `without-body`: Log everything except bodies.
 - `status-at-least`: Log only if the response status is >= minimumStatus.
 - `body-only-if-status-at-least`: Log the request without a body, and then log both the request and response with bodies only if the response status is >= minimumStatus.
...
[.description]
--
Splits log lines into smaller chunks of a given size. A value of 0 (the default) disables chunking.
...
[.description]
--
Truncates the body to a given number of characters and appends "...". A value of -1 (the default) disables truncation.
...
[.description]
--
A list of header names to be obfuscated. By default, "Authorization" is obfuscated.
...
[.description]
--
A list of parameter names to be obfuscated. By default, "access_token" is obfuscated.
...
[.description]
--
The value to be used as a replacement for obfuscated data. The default is "XXX".
...
[.description]
--
Minimum status code to enable logging for strategies that depend on it. This is used by the "status-at-least" and "body-only-if-status-at-least" strategies. The default is 400.
...
[.description]
--
The type of attribute extractor to use. Currently supports "JwtFirstMatchingClaimExtractor" and "JwtAllMatchingClaimsExtractor".
